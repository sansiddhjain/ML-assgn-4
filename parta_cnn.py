...
df = pd.DataFrame(columns=['filter', 'kernel', 'dense_units', 'train_acc', 'val_acc'])
count = 0
df = pd.read_csv('cnn_hyp_tuning.csv')
count = len(df)
print('Starting training')
for filters in [16, 32, 48, 64, 96, 128]:
  for kernel in [3, 4, 5]:
    print('Starting training on - '+str(filters)+', '+str(kernel))
    model = Sequential()
    model.add(Conv2D(filters, (kernel, kernel), activation='relu', input_shape=(28, 28, 1), padding='same'))
    ...
    x = model.fit(X_train, y_train_one_hot, epochs=10, batch_size=128, verbose=2, validation_data=(X_val, y_val_one_hot))
    print x
    ...
    print(str(loss_and_metrics))
    df.loc[count, :] = [filters, kernel, 2048, train_acc, val_acc]
    count += 1
    ...
    df_sub.to_csv('hyp_tuning/submission_'+str(count)+'.csv', index=False)
